<div class="layout-page">
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="container-breadcrumb my-2">
            <h3 class="fw-bold mb-1">Semua Satuan</h3>
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    <div class="col-6">
                        ...
                        <input type="text" name="unit_name" class="form-control" placeholder="contoh : Kilogram">
                    </div>

                    <div class="col-6">
                        <label class="form-label">Satuan type <span class="text-danger">*</span></label>
                        <input type="text" name="unit_type" class="form-control" placeholder="contoh : Kg">
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '#add-action', function (e) { 
            e.preventDefault();
            formModal('add', 'Tambah Satuan', 'Tambah')
        });

        $(document).on('click', '.edit', function(e){
            e.preventDefault();
            formModal('edit', 'Edit Satuan', 'Simpan')

            $.ajax({
                type: "POST",
                url: "<?= base_url() ?>admin/units/get_units/" + $(this).data('id'),
                data: "csrf_token=" + csrf.attr('content'),
                dataType: "JSON",
                success: function (res) {
                    if(res.success == "true"){
                        $('input[name="target"]').val(res.data.unit_id)
                        $('input[name="unit_name"]').val(res.data.unit_name)
                        $('input[name="unit_type"]').val(res.data.unit_type)
                    }
            ...
            Swal.fire({
                html:
                    ...
                    <div>
                        ...
                        <p class="fs-6 mt-2">Anda yakin ingin menghapus satuan ini?</p>
                    </div>`,
                customClass: {
                ...
                if(e.value){
                    $.ajax({
                        type: "POST",
                        url: "<?= base_url() ?>admin/units/action/delete",
        ...
        btnSave.click(function (e) { 
            ...
            if(saveAction == 'add'){
                $.ajax({
                    type: "POST",
                    url: "<?= base_url() ?>admin/units/action/add",
            ...
            if(saveAction == 'edit'){
                $.ajax({
                    type: "POST",
                    url: "<?= base_url() ?>admin/units/action/edit",
                    data: formData.serialize() + "&csrf_token="+ csrf.attr('content'),
                    dataType: "JSON",
                    success: function (res) {
                        notifyAjax(res)
